...
\begin{figure}[t]
    \centering
    \subfigure[Projection in $\mathbb{R}^3$]{
    \label{fig:3dplot}
    \includegraphics[width=0.25\textwidth]{../Figs/3dplot}}
\quad
    \subfigure[$Y_kV$]{
    \label{fig:2dscatter}
    \includegraphics[width=0.25\textwidth]{../Figs/2d-scatter}}
\quad 
\subfigure[$Y_kV_{\perp}$]{
\label{fig:2dscatterorth}
\includegraphics[width=0.25\textwidth]{../Figs/2d-scatter-orth}}
...
%\nonumber &= \frac{1}{2}\sum_k \tr\left(-M_k^{-1}V^T S_kV\right) +
%  \text{tr}\left(VV^T S_k\right)/\sigma_k^2\\
...
\edits{For an arbitrary $p$-vector $\eta$, the asymptotic bias of
  ...
  % An asymptotically lower variance estimator can then be constructed
  % by combining consistent estimators from each of the $K$ groups
  % (e.g. by weighting each of the sample eigenprojection by the inverse
  % Fisher Information).
 ...
 % In this paper we consider initializations
  % based on a weighted average of the eigenprojection matrices
  % associated with each eigenvalue from each group:
  % $ P = \frac{\sum_k \sum_i w_{ik} U_i^{(k)}U_i^{(k)^T}}{\sum_k\sum_i
  %   w_{ik}}$.  In general, $P \notin \mathcal{G}_{p, s}$, i.e. P is
  % not a projection matrix.  Thus, we choose the starting value to be
  % the closest projection matrix
  % $\hat{V}_{\text{init}}\hat{V}_{\text{init}}^T$ to $P$.
  % $\hat{V}_{init}$ is the first $s$ eigenvectors of $P$.
  % $\frac{\ell_i^{(k)}}{\sigma^2}$
...
%       between EM estimates and the true shared subspace for 1000 random initializations (grey) and ``smart'' initialization (red line).  }
% \end{figure}
...
Our goodness of fit measure if based on the fact that when $V$ is a
basis for a shared subspace, then for each group, most of the
non-isotropic variation in $Y_k$ should be preserved when projecting
the data onto this space.  To characterize the extent to which this is
true for different groups, we propose a simple estimator for the
proportion of ``signal'' variance that lies on a given subspace.
Specifically, we use the following statistic for the ratio of the sum
of the first $s$ eigenvalues of $V^T \Sigma_k V$ to the sum of the
first $s$ eigenvalues of $\Sigma_k$:
%
\begin{equation}
\gamma(Y_k: V, \sigma_k^2) = \frac{||Y_kV||^2_F/n_k}{\underset{\widetilde{V} \in \mathcal{V}_{p, s}}{\text{max}}
  ||Y_k\widetilde{V}||^2_F/n_k - B_k }
\label{eqn:ratio}
\end{equation}
%
\noindent where $||\cdot||_F$ is the Frobenius norm and $B_k$ is a
bias correction where $B_k = \sigma_k^2 p / n_k \sum_k
\left(\frac{m_i^{(k)}}{m_i^{(k)}-\sigma^2_k}\right)$ with $m_i^{(k)}$
the positive solution to the quadratic equation 
\begin{equation}
(m_i^{(k)})^2 + m_i^{(k)}(\sigma^2_kp/n_k - \sigma_k^2 - \hat{\lambda}_i^{(k)}) -
\hat{\lambda}_i^{(k)}\sigma_k^2 = 0.
\label{eqn:quad}
\end{equation}
and $\hat{\lambda}_i^{(k)}$ is the $i$-th eigenvalue of $S_k/n_k$.
\edits{
\begin{theorem}
Assume $p/n_k \to \alpha_k$ and $s$ is fixed. If
$\Sigma_k = V\Psi_kV^T + \sigma_k^2I$, then $\gamma(Y_k: V, \sigma_k^2)
\to 1$ as $n_k, p \to \infty$.  
\end{theorem}
}
\edits{
\begin{proof}
Since $s$ is fixed and $n_k$ is growing, the numerator,
$||Y_kV||^2_F/n_k$, is a consistent estimator for the sum of the
eigenvalues of $V^T\Sigma_kV$.  In the denominator,
$\underset{\widetilde{V} \in \mathcal{V}_{p,
    s}}{\text{max}}||Y_k\widetilde{V}||^2_F/n_k$
is equivalent to the sum of the first $s$ eigenvalues of the sample
covariance matrix $S_k/n_k$.  \citet{Baik2006} and others have demonstrated that
asymptotically as $p, n_k \rightarrow \infty$ and $p/n_k = \alpha_k$, $\hat{\lambda}^{(k)}_i$ is
positively biased.  Specifically,
%
\begin{eqnarray}
...
\end{eqnarray}
%
\noindent Replacing $\lambda_i^{(k)}$ by $m_i^{(k)}$ and assuming equality in \ref{eqn:lamAsymp} yields the quadratic equation \ref{eqn:quad}.  The
solution, $m_i^{(k)}$, is an asymptotically (in $n$ and $p$) unbiased
estimator of $\lambda_i^{(k)}$ and
\begin{equation}
\underset{\widetilde{V} \in \mathcal{V}_{p, s}}{\text{max}}
  ||Y_k\widetilde{V}||^2_F/n_k - B_k  \to \sum_{i}^s \lambda_i^{(k)} 
\end{equation}
As such, when the shared subspace model holds both the
numerator and denominator of the goodness of fit statistic converge
almost surely to $\sum_{i=1}^{s}
\lambda_i^{(k)}$.  Therefore $\gamma(Y_k: V, \sigma_k^2)
\to 1$.
\end{proof}
}
% Our motivation for
...
The goodness of fit statistic will be close to one for all
groups when $VV^T$ is a shared subspace for the data and typically smaller if
...
variances $\hat{\sigma}_k^2$ using EM and compute the plug-in estimate
$\gamma(Y_k: \hat{V}, \hat{\sigma}_k^2)$.  When this statistic is
small for some groups, it may suggest that the rank $s$ of the
inferred subspace needs to be larger to capture the variation in all
groups. If $\gamma(Y_k: \hat{V}, \hat{\sigma}_k^2)$ is substantially
larger than 1 for a particular group, it suggests that the inferred subspace
is too similar to the sample principal components from group $k$.  We
investigate these issues in Section \ref{sec:simulation}, by computing the
goodness of fit statistic for inferred subspaces of different
dimensions on a single dataset. In Section \ref{sec:app}, we compute
the estimates for subspaces inferred with real biological data.
...
\begin{figure}[t]
    \centering
    \subfigure[Stein's risk vs $\hat{s}$]{
      \label{fig:sdimension}
      \includegraphics[width=0.3\textwidth]{../Figs/LossVsDimension}}
    \subfigure[$\hat{s}$ = 5]{
      \label{fig:ratio-s5}
      \includegraphics[width=0.3\textwidth]{../Figs/simRatio-s5}}
    \subfigure[$\hat{s}$ = 20]{
        \label{fig:ratio-s20}
        \includegraphics[width=0.3\textwidth]{../Figs/simRatio-s20}}
...
%\todo{Paragraph on implications if spiked covariance model does not hold?}
...
\edits{% Equality is achieved when the
  % eigenvectors from each group are also identical (e.g. the data from each group is
...
$$|\langle\hat{V}_i, V_i\rangle| \to \sqrt{\left(1-\frac{\alpha}{K(\lambda_i - 1)^2}\right) /\left(1 +
    \frac{\alpha}{K(\lambda_i - 1)}\right)} $$
%
% Note that in the likelihood specified in Equation \ref{eqn:likV} , the
% sample covariance matrices, $S_k/n_k$, are sufficient statistics.
% Since the sample covariance matrices are biased for the true
% eigenvectors of $\Sigma_k$ when $p$ and $n$ increase at the same rate
% then $\tr(\hat{V}\hat{V}^TVV^T)/s < 1$ almost surely \citep{}.
...
% This is supported by the results in Table
...
% significant reduction in Stein's risk. 

\begin{figure}[t]
    \centering
    \includegraphics[width=0.7\textwidth]{../Figs/asymptotics}
    \caption{Subspace accuracy $\tr(\hat{V}\hat{V}^TVV^T)/s$ (solid)
      ...
      %     1)^2}\right) /\left(1 + \frac{\gamma}{K(\lambda_i -
      %     1)}\right)$
...
% \begin{align*}
% \hat{V} &= \underset{\widetilde{V} \in \mathcal{V}_{p,
%     s}}{\text{argmax }} \tr\left((I-M_k^{-1})^{1/2}\widetilde{V}^T
%   (\sum_{k=1}^K S_k)\widetilde{V}(I-M_k^{-1})^{1/2}\right)
% \end{align*}
% Where $(I-M_k^{-1})^{1/2}$

% \begin{align*}
% Y &= ZV\psi_k^{1/2}V^T+E\\
% Y^TY &= V\psi_k^{1/2}V^TZ^TZV\psi_k^{1/2}V^T + E^TE + \text{cross terms}
% \end{align*}

% \noindent where $Z$ and $E$ are $n$ by $p$ matrices of iid stanard
% normals.  

% $$ \tr\left((I-M_k^{-1})\widetilde{V}^TVM_k^{1/2}V^TZ^TZVM_k^{1/2}V^T\widetilde{V}\right)$$
%% \subsection{Consistency of $\Psi_k$ and $\sigma_k^2$ given $V$}
...
\begin{figure}[t]
    \centering
    \subfigure[$\gamma(Y_k: \hat{V}, \hat{\sigma_k}^2)$]{
        \label{fig:leukemiaRatio}
        \includegraphics[width=0.45\textwidth]{../Figs/leukemiaRatio-07-07}}
    ~ %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. 
      %(or a blank line to force the subfigure onto a new line)
    \subfigure[95\% Posterior Regions]{
      \label{fig:leukemiaPosterior}
        \includegraphics[width=0.45\textwidth]{../Figs/leukemiaPosterior-07-07}}
...
loadings have opposite signs on the second column of $\hat{V}_1$.
  ...
  \begin{figure}[!ht]
    \centering
    \includegraphics[width=0.45\textwidth]{../Figs/leukemia-biplot-07-07}
    \qquad
\raisebox{1\height}{
\scriptsize
...
\begin{figure}[t]
    ...
    \subfigure[$\gamma(Y_k: \hat{V},\hat{\sigma_k}^2)$]{
        ...
        \includegraphics[width=0.45\textwidth]{../Figs/dmelanRatio-07-07}}
    ~ %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. 
      %(or a blank line to force the subfigure onto a new line)
    \subfigure[95\% Posterior Regions]{
      \label{fig:dmelanPosterior}
      \includegraphics[width=0.45\textwidth]{../Figs/dmelanPosterior-07-07}}
